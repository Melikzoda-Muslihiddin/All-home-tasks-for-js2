const box = document.querySelector('.cardsCategoys') as HTMLDivElement;
const popularBox = document.querySelector('.PopularLaptops') as HTMLDivElement;
const laptopBox = document.querySelector('.PremiumLaptops') as HTMLDivElement;
const paginationBox = document.querySelector('.pagination') as HTMLDivElement;
...
export function ShowCategory(data: any) {
  box.innerHTML = '';
...
export function PopularLaptop(data: any) {
...
export function initLaptops(data: any[]) {
  allData = data.map(p => {
...
function applyFilters() {
  const priceCheckboxes = document.querySelectorAll('.filter-price:checked') as NodeListOf<HTMLInputElement>;
  const brandCheckboxes = document.querySelectorAll('.filter-brand:checked') as NodeListOf<HTMLInputElement>;
  ...
  filteredData = allData.filter(item => {
    ...
    if(selectedPrices.length > 0){
      priceMatch = selectedPrices.some(range => {
        if(range === "2500+") return item.priceNumber >= 2500;
    ...
    if(searchValue) searchMatch = item.name.toLowerCase().includes(searchValue);
...
function applySort(order: 'high' | 'low') {
...
const filterBtn = document.querySelector('.filterbtn') as HTMLButtonElement;
